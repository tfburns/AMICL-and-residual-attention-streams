...
class GPT(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        for block in self.blocks:
            x, prev_values = block(x, prev_values)
        x = self.ln(x)
        if y is not None:
            logits = self.fc(x)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, prev_values):
        residual = x
        x = self.ln1(x)
        x, prev_values = self.attn(x, prev_values)
        x = residual + x
        residual = x
        x = self.ln2(x)
        x = self.ffn(x)
        x = residual + x
        return x, prev_values
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, prev_values, attention_mask=None):
        # x: B, T, H
        ...
        attn_output, attn_weights = self._attn(query, key, value + prev_values, attention_mask) # B, NH, T, E
